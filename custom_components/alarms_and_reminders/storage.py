...
class AlarmReminderStorage:
    ...
    async def async_load(self) -> Dict[str, Dict[str, Any]]:
        """Load all items from storage and return mapping item_id -> item dict."""
        data = await self._store.async_load()
        if not data:
            self._items = {}
            return {}
        # Stored format: {"items": {id: {...}}}
        raw = data.get("items") if isinstance(data, dict) else None
        if not raw:
            self._items = {}
            return {}
        # Return a shallow copy to caller
        self._items = dict(raw)
        _LOGGER.debug("AlarmReminderStorage loaded %d items", len(self._items))
    ...
    async def async_save(self, items: Optional[Dict[str, Dict[str, Any]]] = None) -> None:
        """Persist items mapping to storage immediately."""
        try:
            async with self._lock:
                if items is None:
                    items = dict(self._items)
                # Ensure JSON serializable: convert datetimes to ISO strings if present
                serializable = {}
                for item_id, data in items.items():
                    ...
                    try:
                        # datetime -> isoformat; if already string leave as-is
                        from datetime import datetime
                        if isinstance(sched, datetime):
                            stored["scheduled_time"] = sched.isoformat()
                    except Exception:
                        pass
                    serializable[item_id] = stored

                await self._store.async_save({"items": serializable})
                # keep in-memory copy synced
                self._items = dict(serializable)
                _LOGGER.debug("AlarmReminderStorage saved %d items", len(self._items))
